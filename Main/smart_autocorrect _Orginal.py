...
class RustSmartAutoCorrect(QObject):
    ...
    def _find_closest_keyword(self, word):
        ...
        for keyword in matching_keywords:
            # Only consider keywords of similar length
            if abs(len(keyword) - len(word)) > 3:
                continue
            
            distance = self._levenshtein_distance(word.lower(), keyword.lower())
            
            # Only suggest if distance is small (1-3 characters different)
        ...
        if best_match:
            ...
            # If less than 40% of letters match, don't suggest
            if matches < len(word) * 0.4:
                return None
    ...
    def _handle_enter_key(self):
        """Handle Enter key - add semicolon if needed at end of line."""
        try:
            cursor = self.editor.textCursor()
            block = cursor.block()
            line = block.text().rstrip()
            
            # Skip if line is empty or already has semicolon
            if not line or line.endswith(';') or line.endswith('{') or line.endswith('}'):
                return
            
            # IMPORTANT: Skip if line ends with comma (match arms, array elements, etc.)
            if line.endswith(','):
                return
            
            # Skip if line looks like a match arm (contains =>)
            if '=>' in line:
                return
            ...
            # let statements with assignment
            if line.strip().startswith('let ') and '=' in line:
                needs_semicolon = True
            
            # return statements (without block)
            elif line.strip().startswith('return ') and '{' not in line:
                needs_semicolon = True
            
            # break/continue statements
            elif line.strip() in ['break', 'continue'] or line.strip().startswith('break ') or line.strip().startswith('continue '):
                needs_semicolon = True
            
            # Function calls or assignments
            elif '=' in line and not line.strip().startswith('fn ') and not line.strip().startswith('if ') and not line.strip().startswith('while ') and not line.strip().startswith('for '):
                needs_semicolon = True
            ...
            if needs_semicolon:
                ...
                # Move cursor back to where it was (after the semicolon, ready for new line)
